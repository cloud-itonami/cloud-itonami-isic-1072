...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:product-type :sugar/refined-white :moisture-percent 0.04}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:product-type :sugar/refined-white}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:product-type :sugar/refined-white :moisture-percent 0.04}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:product-type (get-in result [:batches "batch-001"])) :sugar/refined-white))
      (is (= (:moisture-percent (get-in result [:batches "batch-001"])) 0.04)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:product-type :sugar/refined-white}}}
