...
If you discover a security issue in cloud-itonami-isic-1072, **do not open a
...
This repository implements the **Governor** for sugar-refining plant
operations — a deterministic safety-checking layer that gates all proposals
from the LLM/advisor.

Security-critical concerns:
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-processed`, `:so2-residue-exceeded`,
   `:foreign-material-detected`) blocks the proposal unconditionally
